...
public record Page(int lvl, String requireMod, PageElement... elements) {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horAmount, double vertAmount) {
        return handleEvent(mouseX, mouseY, (e, ex, ey) -> e.mouseScrolled(ex, ey, horAmount, vertAmount));
    }
    ...
    public void renderTooltip(GuiGraphics context, int mouseX, int mouseY, int relX, int relY) {
        ...
        for (PageElement element : elements) {
            ...
            if (relY >= currentY && relY <= currentY + height) {
                ...
                if (stack != null) {
                    context.renderTooltip(Minecraft.getInstance().font, stack, mouseX, mouseY);
                    return;
                }
            }
            currentY += height;
